...
import jp.co.tottori.mapper.BookMapper;
...
public class BookService {
	...
	public List<MypageRentalDto> rentalConfirm(RentalDto dto) {
		...
		public List<MypageRentalDto> rentalConfirm(ReserveDto dto) {
			List<MypageRentalDto> rentalBook = bookMapper.rentalConfirm(dto);
			return rentalBook;
		}

		//延滞チェック
		public List<MypageRentalDto> delinquentBook(ReserveDto dto) throws ParseException {
			//１４日前の日付を取得(取得した日以前の場合延滞)
			Date date = new Date();
			Calendar cal = Calendar.getInstance();
			cal.setTime(date);
